...
########################################################
    if g_bConnected:
        printToMessages("You are already connected! {} -".format(g_sock.getpeername()[0]).encode())
    else:
        ...
        try:
            ...
            printToMessages("Error: thrown during connection!")
########################################################
...
########################################################
    if g_bConnected:
        try:
            ...
            g_sock.send(message.encode())
        except:
            printToMessages("Client {} will be disconnected due to error!".format(g_sock.getpeername()[0]).encode())
            disconnect()
    else:
        printToMessages("To send messages, Please connect first!")
########################################################
    ...
    if g_bConnected:
        ...
        try:
            ...
            if data:
                ...
                printToMessages("Error: thrown during data transmission!")
                disconnect()
        except:
            pass
########################################################
